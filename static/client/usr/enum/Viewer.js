...
Ext.define('App.usr.enum.Viewer', {
	...
	buildVis : function() {
		...
		var complexes, reactions, restingStates, initial_complexes;

		initial_complexes = _.reduce(data['initial_complexes'], function(memo, c) {
			memo[c.name] = true;
			return memo;
		}, {});
		...
		var complexMap = _.reduce(complexData, function(memo, x, i) {
			memo[x.name] = x;
			return memo;
		}, {});

		// Build data for the reaction noes
		var reactionData = _.map(reactions, function(x, i) {
			x._type = 'reaction';
			x.name = 'reaction_' + i;
