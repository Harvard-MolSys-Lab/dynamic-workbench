...
var DD = function() {
	...
	/**
	 * Prints an error message `msg` to the console and raises `exception`
	 * @param {String} msg
	...
	function pairscore(seq1, len1, seq2, len2) {
		...
		if(( Cmatrix = new Array(len1  /* double* */
			...
			error("Insufficient memory for score calculations!\n");
		}
	...
	/**
	 * Copies values of {@link rules} hash to local variables
	 * @private
	 */
	function copyRules() { 
		rule_4g = rules.rule_4g, rule_6at = rules.rule_6at, rule_ccend = rules.rule_ccend, rule_ming = rules.rule_ming, rule_init = rules.rule_init, rule_lockold = rules.rule_lockold, rule_targetworst = rules.rule_targetworst, rule_gatc_avail = rules.rule_gatc_avail;
	...
	function loadFile(fileContents, newDomains) {
		...
		for( j = 0; j < num_domain; j++) {
			...
			i = domain_length[j] + 1;
			domain_importance[j] = parseInt(line[1]);
			domain_gatc_avail[j] = parseInt(line[2]);
			
			if(isNaN(domain_importance[j]))   {
				printf("Input file corrupted.  Non-numeric parameter for importance of domain %d.\n", (j + 1));
				throw 'Error';
			} 
			if(isNaN(domain_gatc_avail[j]))   {
	...
	function mutate() {
		...
		* *****************************/
		...
		if(score < old_score || (score == old_score && int_urn(1, 100) <= 80)) {
			total_mutations = total_mutations + num_mut;
			if(score != old_score) bored_mutations = 0;
			mutation_delta = (old_score - score);  
			mutation_flux = 0.9*mutation_flux + 2*mutation_delta;  
	...
	_.extend(this, {
		...
		mutate : mutate,
		mutateUntil:  function(stop) {
			do {
				this.mutate();
			} while(!stop(this))
		},
		mutateWhile: function(keepGoing) {
			do {
				this.mutate()
		...
		updateRules : function(newRules) {
			newRules = _.defaults(newRules,rules);
		...
		updateParams : function(newParams) {
			newParams = _.defaults(newParams,params);
		...
		getMutationFlux : function() {  
			return mutation_flux 
		},
		getBoredMutations : function() {
			return bored_mutations;
		},
		getMutationDelta : function() {
