 ...
 *
 * This specific class uses the Zend_Oauth class
 */
class Dropbox_OAuth_Zend extends Dropbox_OAuth {
    ...
    public function __construct($consumerKey, $consumerSecret) {
		...
		*/
        ...
        $this->OAuth = new Zend_Oauth_Consumer($config); //new OAuth($consumerKey, $consumerSecret,OAUTH_SIG_METHOD_HMACSHA1,OAUTH_AUTH_TYPE_URI);
    ...
    public function setToken($token, $token_secret = null) {
		...
		$this->lastToken = new Zend_Oauth_Token_Access();
		$this->lastToken->setToken($this->oauth_token);
		$this->lastToken->setTokenSecret($this->oauth_token_secret);
    }
    ...
    public function fetch($uri, $arguments = array(), $method = 'GET', $httpHeaders = array()) {
        ...
        try { 
            if(!$this->client) {
            	$this->client = $this->lastToken->getHttpClient(array(
