...
# 2a: Adds DD output and better handling of polarity errors.
# 2b: new input format has each line start with a string naming the motif, and renumbers segments so that no integer values are skipped.
...
for i in range(numm):
  ...
  m.append({})
  m[i]['name'] = linelst[0]
  m[i]['type'] = int(linelst[1])
  m[i]['out'] = []
  for j in range(len(mtable[m[i]['type']]['outport'])):
    if int(linelst[2+j]) == 0:
       m[i]['out'].append([])
    else:
       m[i]['out'].append(int(linelst[2+j]))
  ...
  for j in range(seglst[m[i]['type']]-1):
    for k in range(j+1,seglst[m[i]['type']]):
      if labels[m[i]['ofs']+j]==-labels[m[i]['ofs']+k] and cmat[m[i]['ofs']+j][m[i]['ofs']+k]==0:
        print 'Complementarity warning: motif ' + repr(i) + '[' + m[i]['name'] + '], segments ' + repr(j) + ' and ' + repr(k)

# squeeze the segment labeling down to eliminate missing integers:
# first make a list of all unique segment labels
unqlst = []
for i in range(len(labels)):
  if unqlst.count(abs(labels[i]))==0:
    unqlst.append(abs(labels[i]))
unqlst.sort()
# now go through all the integers, and each time you find one missing from unqlst, rename the current highest label with that missing label
j = len(unqlst)-1
for i in range(1,len(unqlst)+1):
  if unqlst.count(i)==0:
    for k in range(len(labels)):
      if abs(labels[k])==unqlst[j]:
        labels[k] = signum(labels[k])*i
    j = j-1
...
for i in range(len(m)):
  ...
  if len(lst)==0:
    fnup.write('structure ' + m[i]['name'] + ' = U' + repr(mtable[m[i]['type']]['numsegs']*SEGLEN) + '\n')
  else:
    lst2 = []
    for j in lst:
      if j != []:
